  ...
  (function(jQuery) {
    jQuery.widget('IKS.hallobutton', {
      ...
      _createButton: function(id, command, label, icon) {
        ...
        return jQuery("<button id=\"" + id + "\"        class=\"" + (classes.join(' ')) + "\" title=\"" + label + "\">          <span class=\"ui-button-text\">            <i class=\"ac-" + icon + "\"></i>          </span>        </button>");
      }
    });
  ...
  (function(jQuery) {
    return jQuery.widget('IKS.hallodropdownbutton', {
      ...
      _prepareButton: function() {
        ...
        buttonEl = jQuery("<button id=\"" + id + "\"       class=\"" + (classes.join(' ')) + "\" title=\"" + this.options.label + "\">       <span class=\"ui-button-text\"><i class=\"ac-" + this.options.icon + "\"></i></span>       </button>");
        if (this.options.cssClass) {
...
(function() {
  (function($) {
    return $.widget('IKS.hallotextcolor', {
      options: {
        editable: null,
        toolbar: null,
        spectrumOptions: {},
      ...
      populateToolbar: function(toolbar) {
        // callback to get current selection
        function addSelectionContainer() {
          if (document.selection) {
            document.execCommand('pasteHTML', null, '<span class="awe-editting-color">' + document.selection() + '</span>');
          }
          else if (document.getSelection) {
            document.execCommand('insertHTML', null, '<span class="awe-editting-color">' + document.getSelection() + '</span>');
          }
        }

        var button = $('<span><input class="color-value" value""></span>').addClass(this.widgetName),
            widget = this,
            changed = false,
            spectrumOptions = this.options.spectrumOptions,
            beforeShowCallback = spectrumOptions.beforeShow && $.type(spectrumOptions.beforeShow) == 'function' ? spectrumOptions.beforeShow : false,
            changeCallback = spectrumOptions.change && $.type(spectrumOptions.change) == 'function' ? spectrumOptions.change : false,
            moveCallback = spectrumOptions.move && $.type(spectrumOptions.move) == 'function' ? spectrumOptions.move : false,
            hideCallback = spectrumOptions.hide && $.type(spectrumOptions.hide) == 'function' ? spectrumOptions.hide : false;

        // assign callback
        spectrumOptions.beforeShow = function(color) {
          // reset flag to define color is changed
          changed = false;

          // add wrapper for selection change text color
          addSelectionContainer();

          // implement custom function
          if (beforeShowCallback)
            beforeShowCallback(color);

          // keep toolbar show when change color
          widget.options.editable.keepActivated(true);
        }; 
        ...
        spectrumOptions.move = function(color) {
          var colorString = color ? color.toRgbString() : '';
          $('.awe-editting-color', widget.options.editable.element).css('color', colorString);
        ...
        spectrumOptions.hide = function(color) {
          var colorString = color ? color.toRgbString() : '';
          if (changed)
            $('.awe-editting-color', widget.options.editable.element).css('color', colorString).removeClass('awe-editting-color');
          else
            document.execCommand('undo', null);

          // reset toolbar is closed by normal
          widget.options.editable.element.focus();
        ...
        // init color picker
        $('> input', button).spectrum(spectrumOptions);
        button.hallobuttonset();
...
(function() {
  (function($) {
    return $.widget('IKS.hallobackgroundtcolor', {
      options: {
        editable: null,
        toolbar: null,
        spectrumOptions: {},
      ...
      populateToolbar: function(toolbar) {
        ...
        spectrumOptions.move = function(color) {
          // implement change background selection color
          var colorString = color ? color.toRgbString() : '';
          document.execCommand('backColor', null, colorString);
